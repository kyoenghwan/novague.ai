...
import { UXFlow, Screen } from '@/types';
    ...
    const { nodes, edges } = useMemo(() => {
        ...
        analysis.screens.forEach((screen, index) => {
            ...
            layoutNodes.push({
                id: screen.id,
                position: { x, y },
                data: { label: screen.name, type: screen.type },
                style: {
            ...
            <div className="flex-none p-4 pb-0">
                <div className="flex items-center justify-between mb-2">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">AI Architect defined {analysis.screens.length} screens and {analysis.flows.length} flows.</p>
                    </div>
                        ...
                        <TabsContent value="screens" className="flex-1 overflow-y-auto p-4 space-y-3">
                            {analysis.screens.map(screen => (
                                <div key={screen.id} className="p-3 border rounded-lg bg-card hover:bg-muted/50 transition-colors cursor-pointer group">
                                    ...
                                    <div className="text-xs text-muted-foreground mb-2">{screen.description}</div>
                                </div>
                            ))}
                        </TabsContent>
